...
\sol{
    ...
    On sait que : $T \# f = g$
    Donc, $\forall \varphi$ continue : $\displaystyle \int {(\varphi \circ T)(x)} \: d{f(x)} = $
    $\displaystyle \int {\varphi(x)} \: d{g(x)}$\\
    D'autre part, $\forall \varphi$ continue : $\displaystyle \int {(\varphi \circ S)(x)} \: d{(f \circ \tau_{a})(x)} = $
    $\displaystyle \int {\varphi (T(x-a) + b)} \: d{f(x-a)} = $\\
    On pose,$y = x+b-a$



    $\displaystyle \int {\varphi(y)} \: d{(g \circ \tau_{b})(y)}$
}
