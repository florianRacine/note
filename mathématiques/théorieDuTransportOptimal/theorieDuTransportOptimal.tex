...
\right.$\\
    ...
    $\tau_{c}^{M} (\mu , \nu) = \displaystyle\int_{\mathbb{R}} {C(a,x)} \: d{\mu (x)}$\\
    D'autre part : \\
...
\cor{Les plans de transferts optimaux sont caractérisés par leur support}{
...
\dfn{Distance}{ 
    On appelle distance sur un ensemble E toute application d
    définie sur le produit $E^{2} = E \times E$ et à valeurs dans l'ensemble
    $\mathbb{R}_{+}$ des réels positifs ou nuls, vérifiant les propriétés suivantes :\\
    \bf{Symétrie} : $$\forall (a,b) \in E^{2}, d(a,b) = d(b,a)$$
    \bf{Séparation} : $$\forall (a,b) \in E^{2}, d(a,b) = 0 \Leftrightarrow a = b$$
    \bf{Inégalité triangulaire} : $$\forall (a,b,c) \in E^{3}, d(a,c) \le d(a,b) + d(b,c)$$
}

\dfn{ $\mathcal{P}_{p}(\mathbb{R}^{n})$ }{
    $\displaystyle \mathcal{P}_{p}(\mathbb{R}^{n}) = \{ \mu \in \mathcal{P} (\mathbb{R}^{n}) | \exists x_{0} \in \mathbb{R}^{n} | 
    \int {[d(x_0 , x)]^{p}} \: d{\mu (x)} < \infty \}$\\
    $\mathcal{P}_{p}(\mathbb{R}^{n})$
    est l'ensemble des mesures de probabilités qui ont un moment d'ordre p fini.
}

\nt{
    On note $\tau_{p} (\mu, \nu) = \tau_{dp} (\mu,\nu)$ le cout associé au transfert optimal de Kantorovitch entre $\mu$ et $\nu$.\\
}

\nt{
    \begin{enumerate}
        \item 
    Si $\mu \in \mathcal{P}_{p}(\mathbb{R}^{n})$ alors $\forall y \in \mathbb{R}^{n}$,
    $\displaystyle \int {d(y,x)^{p}} \: d{\mu(x)} < \infty$\\
        \item 
    Si d est borné $\mathcal{P}_{p}(\mathbb{R}^{n}) = \mathcal{P}(\mathbb{R}^{n})$ \\
    En effet si $\exists x_{0}\in\mathbb{R}^{n}$ tq $\int {[d(x_0 , x)]^{p}} \: d{\mu(x)} < \infty$\\
    alors $\forall y \in \mathbb{R}^{n}, d(y,x) = d(y-x_0 + x_0, x) \le d(y-x_0,x) + d(x_0 , x) = d(y-x, x_0) + d(x, x_0 )$\\
        \item
    Evidemment $\mathcal{P}_{p}(\mathbb{R}^{n}) \subset \mathcal{P}(\mathbb{R}^{n})$
    \end{enumerate}
}
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{fig1chap2}
    \label{}
\end{figure}
...
\dfn{}{
    ...
    \nt{
        Remarque :\\
        $W_{p}(\mu, \nu) = [\tau_{p}(\mu,\nu)]^{\frac{1}{p}}$\\
        Soit $0 \le p \le 1, W_p (\mu, \nu) = \tau_{p} (\mu, \nu)$\\
        Pour $p=1$\\
        $$ || f-g ||_{\mathcal{L}^{1} (\mathbb{R})} = \int_{\mathbb{R}} {|f-g|(x)} \: d{x}$$
        $$ || f-g ||_{\mathcal{L}^{1} (\mathbb{R})} = \int_{\mathbb{R}} {f(x)} \: d{x} + \int_{\mathbb{R}} {g(x)} \: d{x}$$ (car f et g sont à supports disjoints)
        $$ || f-g ||_{\mathcal{L}^{1} (\mathbb{R})} = \int_{a-\frac{1}{2n}}^{a+\frac{1}{2n}} {n} \: d{x} + \int_{b-\frac{1}{2n}}^{b+\frac{1}{2n}} {n} \: d{x} $$
        $$ || f-g ||_{\mathcal{L}^{1} (\mathbb{R})} = 2$$
        $$ W_1 (\mu , \nu) = |a-b|$$
    }
}

\thm{}{
    $W^{p}$ est une distance $(p \ge 1)$ sur $\mathcal{P}_{p}(\mathbb{R}^n)$\\
    On l'appelle distance de \textbf{Wasserstein} ou distance de \textbf{Monge-kantorovitch}.
}

\begin{myproof}

\mlenma{Lemme de Gluing Lemma}{
    $\mu_1 , \mu_2 , \mu_3$ 3 mesures de probabilités.\\
    On note $\pi_{12} \in \Pi (\mu_1 , \mu_2)$\\
    Et, $\pi_{23} \in \Pi (\mu_2 , \mu_3)$\\
    Alors, $\exists \pi \in \mathcal{P}(\mathbb{R}^{n} \times \mathbb{R}^{n}
    \times \mathbb{R}^{n})$ qui a pour marginales $\pi_{12}$ sur
    $\mathbb{R}^{n} \times \mathbb{R}^{n}$ et $\pi_{23}$ sur $\mathbb{R}^{n}
    \times \mathbb{R}^{n}$
}
...
\dfn{Inégalité Triangulaire}{
    ...
    On note $\pi$ la mesure de proba sur $\mathbb{R}^{n} \times \mathbb{R}^{n} \times \mathbb{R}^{n}$ définie dans le lemme définit ci-dessus.\\
...
\begin{enumerate}
    \item 
        $W_p$ est symétrique, positive et $W_p (\mu , \nu) = 0$ (car d distance, donc $d^{p}(x,y) = 0$ ssi $x=y$)\\
    \item
        Si $W_p (\mu , \nu ) = 0$ alors pour $\pi$ de transfert optimal qui envoie $\mu$ sur $\nu$ on a:
        $$\int {d(x,y)^{p}} \: d{\pi(x,y)} = 0$$
        d est une distance, donc $SUP(\pi) \subset \{x=y \}$\\
        Donc $\forall \varphi \in \mathcal{C}^{0}(\mathbb{R}^{n}), \int {\varphi
        (x)} \: d{\mu (x)} = \int {\varphi (x)} \: d{\pi(x,y)} = \int {\varphi (y)} \:
        d{\pi(x,y)} =  \int {\varphi (y)} \: d{\nu (y)}$\\
        Ainsi, $\mu = \nu$
    \item
        $W_p (\mu_1 , \mu_3) 
        \le [\int_{\mathbb{R}^{n}_1 \times \mathbb{R}^{n}_3} {d(x_1,x_3)^{p}} \: d{\pi_{13} (x_1 , x_3)}]^{\frac{1}{p}}
        = [\int_{\mathbb{R}^{n}_1 \times \mathbb{R}^{n}_{2} \times \mathbb{R}^{n}_{3}} {d(x_1,x_3)^{p}} \: d{\pi(x_1 , x_2 , x_3)}]^{\frac{1}{p}}$\\ 
        $\le [\int_{\mathbb{R}^{n}_1 \times \mathbb{R}^{n}_{2} \times
        \mathbb{R}^{n}_{3}} {[d(x_1,x_2) + d(x_2, x_3)]^{p}} \: d{\pi(x_1 , x_2 ,
        x_3)}]^{\frac{1}{p}}$\\ 
        Inégalité de Minkowski dans $\mathcal{L}^{p}$\\
        $\le [\int_{\mathbb{R}^{n}_1 \times \mathbb{R}^{n}_{2} \times \mathbb{R}^{n}_{3}} {d(x_1,x_2)^{p}} \: d{\pi(x_1 , x_2 , x_3)}]^{\frac{1}{p}}
        + [\int_{\mathbb{R}^{n}_1 \times \mathbb{R}^{n}_{2} \times \mathbb{R}^{n}_{3}} {d(x_2,x_3)^{p}} \: d{\pi(x_1 , x_2 , x_3)}]^{\frac{1}{p}}$
        $= W_p (\mu_1 , \mu_2) + W_p (\mu_2 , \mu_3)$\\
        Ainsi, $W_p (\mu_1 , \mu_3) \le W_p (\mu_1 , \mu_2) + W_p (\mu_2 , \mu_3)$\\
\end{enumerate}
...
\ex{}{
   ...
   $W_{2}$ est la distance de Wasserstein quadratique.\\
    Si $\mu \in \mathcal{P}_{2}(\mathbb{R}^{n}), a \in \mathbb{R}^{n}$ alors, $W_2 (\mu, \delta_a ) = \int_{\mathbb{R}^n} {||x-a||^{2}} \: d{\mu (x)}$\\
    En effet une seul application T envoie $\mu$ sur $\delta_a$ : $T(x) = a \forall x \in \mathbb{R}^{n}$\\
...
\ex{}{
    $p=1$\\
    $\tau_1$ s'apppelle la distance de Rubinstein-Kantorovitch.
}

\mprop{}{
    $$ W_1 (\mu , \nu) = SUP_{||\varphi ||_{Lip}\le 1} \int_{\mathbb{R^{n}}} {\varphi (x)} \: d{(\mu - \nu)(x)} $$
    Avec $||\varphi||_{Lip}= SUP_{(x,y),x \neq y} |\frac{\varphi(x) - \varphi(y)}{x-y}|$
}
...
\dfn{Cumulative distribution function : CDF}{
    $\mu \in \mathcal{P}(\mathbb{R})$
    On définit sa CDF par :
    $F(x) = \int_{]-\infty, x]} {} \: d{\mu} {} = \mu(]-\infty, x]) $
    Avec F continue à droite.\\
    $F(-\infty) = 0$\\
    $F(+\infty) = 0$
}
...
\subsection{Barycentres de Wasserstein}

\ex{
    $\mu = \frac{1}{2}(\delta_1 + \delta_3)$\\
    Calculer $F, F^{-1}$ et les tracer.\\
    \begin{enumerate}
        \item   
...
\dfn{}{
    $\mu^{*} = INF_{\mu \in \mathcal{P}_2 (\mathbb{R}^{n})} \sum_{i=1}^{n} \lambda_i W_{2}^{2} (\mu , \mu_i)$\\
